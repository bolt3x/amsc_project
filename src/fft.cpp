...
void FFTGenerator::OPENMP_FFT(const unsigned long &num_threads ){
    ...
    #pragma omp for
        for(size_t j =1; j <= static_cast<size_t>(std::log2(n)); ++j){
            ...
            unsigned int d2 = d >> 1; // m2 = m/2
            // principle root of nth complex root of unity.
            std::complex<double> w_d(std::polar(1.0, static_cast<double>(- 2)*(M_PI/static_cast<double>(d))));
            std::complex<double> w(1.0,0.0);
...
std::vector<std::complex<double>> FFTGenerator::getOpenMPT()const{
    return m_openmp;
}

// element-of-InvT getter...

std::complex<double> FFTGenerator::getOpenMPT(unsigned int const pos) const{
    return m_openmp[pos];
}
...
// by-element OpenMP setter

void FFTGenerator::setOpenMPT(unsigned long const pos, std::complex<double> const val){
    m_openmp[pos] = val;
}

//global OpenMP setter

void FFTGenerator::setOpenMPT(std::vector<std::complex<double>> y){
    m_openmp = y;
}
